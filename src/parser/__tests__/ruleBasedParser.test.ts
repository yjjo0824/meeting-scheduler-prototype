...
describe('parseChips — 복수 조건 접속사 분할(§6, 이 목록만 지원: 쉼표·과/와/이랑/랑/그리고)', () => {
  it('"목요일 오전이랑 수요일 오후는 안 돼요" → 불가 칩 2개', () => {
    ...
    expect(result[1]).toMatchObject({ day: '수', hours: [13, 14, 15, 16, 17] })
  })
...
describe('parseChips — 시간대 의미 규칙(오후 = 정오 이후 슬롯 전체, 구체적 시각 우선)', () => {
  it('"금요일 오후 안 돼요" → 금 13·14·15·16·17 불가(점심 제외 격자에서 오후는 13시부터)', () => {
    const result = parseChips({ raw: '금요일 오후 안 돼요', calendarEvents: [], grid: RAW_SEED.grid })
    expect(result).toHaveLength(1)
    expect(result[0]).toMatchObject({ type: '불가', day: '금', hours: [13, 14, 15, 16, 17] })
  })

  it('"금요일 오후 2시 이후 안 돼요" → 구체적 시각이 우선해 금 14·15·16·17 불가', () => {
    const result = parseChips({ raw: '금요일 오후 2시 이후 안 돼요', calendarEvents: [], grid: RAW_SEED.grid })
    expect(result).toHaveLength(1)
    expect(result[0]).toMatchObject({ type: '불가', day: '금', hours: [14, 15, 16, 17] })
  })

  it('"금요일 13시 안 돼요" → 금 13 단일 슬롯 불가', () => {
    const result = parseChips({ raw: '금요일 13시 안 돼요', calendarEvents: [], grid: RAW_SEED.grid })
    expect(result).toHaveLength(1)
    expect(result[0]).toMatchObject({ type: '불가', day: '금', hours: [13] })
  })

  it('"수요일 오후 3시부터 안 돼요" → "부터"도 그 시각부터 남은 슬롯 전체(수 15~17)', () => {
    const result = parseChips({ raw: '수요일 오후 3시부터 안 돼요', calendarEvents: [], grid: RAW_SEED.grid })
    expect(result).toHaveLength(1)
    expect(result[0]).toMatchObject({ type: '불가', day: '수', hours: [15, 16, 17] })
  })

  it('시드 관용 표현 "오후 내내"는 도윤 원문 재현(수 14~17)을 그대로 유지한다', () => {
    const doyun = person('doyun')
    const result = parseChips({ raw: doyun.response.raw!, calendarEvents: doyun.calendar, grid: RAW_SEED.grid })
    expect(result[0]).toMatchObject({ type: '불가', day: '수', hours: [14, 15, 16, 17] })
  })
})
