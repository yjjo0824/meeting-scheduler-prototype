...
describe('PersonDetailPanel — 출처·조건·진입 CTA(IMPLEMENTATION_SPEC §4.2, R4)', () => {
  ...
  it('참여자 화면 보기 버튼이 항상 표시된다', () => {
    const html = render('minjun', true)
    expect(html).toContain('참여자 화면 보기')
  })
  ...
  it('응답 전인 도윤은 캘린더 조건(월17시·참석 어려움)만 보이고, 응답 칩(회피·조정가능)은 아직 보이지 않는다(R7)', () => {
    const html = render('doyun', false)
    // 캘린더 출처는 응답 여부와 무관하게 항상 알려진 정보라 그대로 보인다.
    expect(html).toContain('월 17시')
    expect(html).toContain('캘린더에서 확인')
