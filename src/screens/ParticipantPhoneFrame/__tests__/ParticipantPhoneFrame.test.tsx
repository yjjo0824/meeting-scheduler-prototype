...
describe('ParticipantPhoneFrame — 5층 구조(맥락→확인→진술→검수→제출)', () => {
  ...
  it('맥락 헤더: 요청자·회의명·범위·기한이 표시된다', () => {
    expect(html).toContain('지원 님이 요청했어요')
    expect(html).toContain('목요일까지 답변해 주세요')
    expect(html).toContain(RAW_SEED.meeting.title)
    expect(html).toContain(RAW_SEED.meeting.response_deadline)
  })

  it('캘린더 프리필: 섹션 제목·월요일 일정 카드·빈 요일 접힘 표기가 보인다(12D-1)', () => {
    expect(html).toContain('캘린더에서 가져왔어요')
    expect(html).toContain('정정하기')
    expect(html).toContain('화–금에는 등록된 일정이 없어요')
  })

  it('자연어 입력 질문·보조 문구가 정확히 표시된다', () => {
    expect(html).toContain('여기에 없는 일정이 있나요?')
    expect(html).toContain('피하고 싶은 시간도 편하게 적어주세요.')
  })

  it('칩 검수 제목·보조 문구가 표시된다', () => {
    expect(html).toContain('이렇게 이해했어요')
    expect(html).toContain('조건이 다르면 ‘조건 바꾸기’를 눌러주세요.')
  })

  it('응답 전에는 칩 검수 영역이 비어 있다(seed의 정답 칩이 미리 노출되지 않음)', () => {
    expect(html).toContain('아직 추가한 조건이 없어요')
    // 12C-12.7: 빈 상태 문구는 캘린더 빈 상태와 동일한 회색 박스(PhoneEmptyNotice) 안에 있다 —
    // 문구 직전의 여는 태그가 박스 클래스를 갖는지 확인한다.
    const emptyIndex = html.indexOf('아직 추가한 조건이 없어요')
    const tagStart = html.lastIndexOf('<p', emptyIndex)
    const tagEnd = html.indexOf('>', tagStart)
    expect(html.slice(tagStart, tagEnd)).toContain('bg-surface-muted')
    expect(html.slice(tagStart, tagEnd)).toContain('rounded-chip')
    expect(html).not.toContain('외부 미팅')
    expect(html).not.toContain('웬만하면')
  })

  it('신뢰 문구(주최자 이름 파생)는 문장 단위 2줄로 고정되고, 제출 CTA가 하단 고정 영역에 존재한다(12C-12.6)', () => {
    // "주최자" 역할 명칭 대신 실제 주최자 이름(지원)이 seed에서 파생되어 들어간다.
    // 문장 중간 자동 줄바꿈 대신 <br/>로 문장 단위 2줄 — 두 문장 사이에 줄바꿈 요소가 있어야 한다.
    const line1 = html.indexOf('지원 님에게는 시간 조건만 보여요.')
    const br = html.indexOf('<br/>', line1)
    const line2 = html.indexOf('작성한 문장과 이유는 보이지 않아요.')
    expect(line1).toBeGreaterThan(-1)
    expect(br).toBeGreaterThan(line1)
    expect(line2).toBeGreaterThan(br)
    expect(html).toContain('이대로 응답하기')
    // CTA 아래 보조 문구는 제거됐다(12C-12.6 항목 2 — 수정 안내는 제출 완료 화면이 담당).
    expect(html).not.toContain('확정 전까지 언제든 수정할 수 있어요')
    // 입력 콘텐츠는 스크롤 영역에, 신뢰 문구+CTA는 그 밖(하단 고정)에 있다.
    const scrollEnd = html.indexOf('</div><div class="space-y-2 border-t border-border pt-3">')
...
describe('ParticipantPhoneFrame — 이미 응답한 사람을 자유 모드에서 다시 열면 "제출 완료" 요약이 먼저 보인다(IMPLEMENTATION_SPEC §5)', () => {
  it('민준을 다시 열면 편집 폼이 아니라 응답 요약 + 수정하기 진입점이 보인다', () => {
    ...
    expect(html).not.toContain('여기에 없는 일정이 있나요?')
...
describe('ParticipantPhoneFrame — 다시 조율 상태(신고 후 재오픈, 파생 상태)', () => {
  ...
  it('12C-12.2: 재조율 상태의 폰 화면 상단에 주최자 이름이 파생된 알림 배너가 뜬다', () => {
    let state = rescheduleState()
    state = appReducer(state, { type: 'OPEN_PHONE_FRAME', personId: 'seoyeon' })
    const html = render(state)

    // 주최자 이름(지원)은 seed의 organizer에서 파생 — 배너는 맥락 헤더보다 위에 있다.
    const bannerIndex = html.indexOf('지원 님이 회의 시간을 다시 조율하고 있어요')
    const headerIndex = html.indexOf('지원 님이 요청했어요')
    expect(bannerIndex).toBeGreaterThan(-1)
    expect(bannerIndex).toBeLessThan(headerIndex)
  })

  it('12C-12.2: 재조율 상태가 아니면 알림 배너가 없다', () => {
    let state = buildInitialState()
    state = appReducer(state, { type: 'OPEN_PHONE_FRAME', personId: 'seoyeon' })
    expect(render(state)).not.toContain('다시 조율하고 있어요')
  })
...
describe('ParticipantPhoneFrame — dialog 접근성 배선(12B-2)', () => {
  ...
  it('확정 후(잠금) 상태: 신고 버튼이 최초 포커스 표식을 갖고, 닫기 버튼은 없다(12C-12.6 — 닫기는 배경 탭/Esc)', () => {
    let state = buildInitialState()
    state = appReducer(state, { type: 'OPEN_PHONE_FRAME', personId: 'seoyeon' })
    state = appReducer(state, { type: 'CONFIRM_MEETING', groupKey: 'k', slot: { day: '금', hour: 13 }, excluded: [] })
    const html = renderToStaticMarkup(
    ...
    expect(html.slice(markerIndex, buttonEnd)).toContain('이 시간에 참석하기 어려워졌어요')
    expect(html).not.toContain('>닫기<')
  })

  it('잠금 + 신고 완료 상태: 신고 버튼이 disabled라 포커스 대상에서 제외된다(getInitialFocus가 패널로 폴백 — :not([disabled]) 셀렉터)', () => {
    let state = buildInitialState()
    state = appReducer(state, { type: 'OPEN_PHONE_FRAME', personId: 'seoyeon' })
    state = appReducer(state, { type: 'CONFIRM_MEETING', groupKey: 'k', slot: { day: '금', hour: 13 }, excluded: [] })
    state = appReducer(state, { type: 'REPORT_UNAVAILABLE', personId: 'seoyeon' })
    ...
    // 표식은 있지만 같은 버튼이 disabled — 실제 폴백(:not([disabled]) → 패널)은 SSR로 재현
    // 불가하므로 셀렉터의 전제(표식+disabled 공존)만 구조적으로 확인한다.
    const markerIndex = html.indexOf('data-phone-focus-target="true"')
    const tagStart = html.lastIndexOf('<button', markerIndex)
    const tagEnd = html.indexOf('>', markerIndex)
    expect(html.slice(tagStart, tagEnd)).toContain('disabled')
  })

  it('12C-12.6: 닫기 버튼은 어떤 상태에도 없고, 배경 딤 탭이 닫기를 담당한다(코드 검토 보완: 클릭은 SSR로 재현 불가)', () => {
    // 배경 onClick은 이제 투어 여부와 무관하게 항상 CLOSE_PHONE_FRAME을 디스패치한다 —
    // 투어 중 실수로 닫혀도 리마인드 버튼으로 재진입 가능(전진 조건은 상태 기반). Esc 닫기는
    // 별도 keydown 핸들러가 유지한다(둘 다 코드 검토로 확인).
    const opened = appReducer(buildInitialState(), { type: 'OPEN_PHONE_FRAME', personId: 'doyun' })
    const html = renderToStaticMarkup(
      <AppProvider initialState={opened}>
        <ParticipantPhoneFrame />
  ...
  it('12C-12.6 재진입: 배경 탭으로 닫힌 뒤 리마인드로 다시 열면 투어 단계가 깨지지 않고 이어진다(리듀서 레벨)', () => {
    // 투어 2단계(폰 프레임)에서 실수로 닫힘 → 미제출이므로 2단계 완료 조건은 여전히 false,
    // 재오픈하면 같은 자리에서 이어간다.
    let state = { ...buildInitialState(), tour: { active: true, stepIndex: 1 } }
    state = appReducer(state, { type: 'OPEN_PHONE_FRAME', personId: 'doyun' })
    state = appReducer(state, { type: 'CLOSE_PHONE_FRAME' })
    expect(state.tour).toEqual({ active: true, stepIndex: 1 })
