...
describe('shouldAutoNavigateToTradeoff — 12C-5: 투어 자동 전환 판정(진행 버그 근본 원인 회귀 가드)', () => {
  ...
  it('12C-12.2: 트레이드오프 단계까지는 전환하고, 그 단계를 지난 뒤(확정 단계)에는 전환하지 않는다', () => {
    // 비트3 진입 시점(stepIndex 2 = tradeoff 단계)에는 자동 전환이 살아 있다.
    const atTradeoffStep = { ...readyState(), tour: { active: true, stepIndex: 2 } }
    expect(shouldAutoNavigateToTradeoff(atTradeoffStep, 3)).toBe(true)
    // 확정 후 "다시 조율하기" 확인으로 확정이 풀린 상태(stepIndex 3, 전원 응답 + host + 미확정
    // 재성립)에서는 전환하지 않는다 — host에 머물러 확정 카드가 잠정 추천 카드로 되돌아오는
    // 것을 볼 수 있어야 한다(대화상자 본문과 동작 일치).
    let rescheduled = { ...readyState(), tour: { active: true, stepIndex: 3 } }
    rescheduled = appReducer(rescheduled, {
      type: 'CONFIRM_MEETING',
      groupKey: 'k',
      slot: { day: '금', hour: 13 },
      excluded: [],
    })
    rescheduled = appReducer(rescheduled, { type: 'REOPEN_FOR_RESCHEDULE' })
    expect(rescheduled.screen).toBe('host')
    expect(shouldAutoNavigateToTradeoff(rescheduled, 3)).toBe(false)
  })
